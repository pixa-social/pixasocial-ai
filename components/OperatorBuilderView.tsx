...
import { Operator, Persona, RSTProfile, ViewName, UserProfile, AIOperatorEffectivenessAnalysis } from '../types';
import { Card } from './ui/Card';
import { Button } from './ui/Button';
import { LoadingSpinner } from './ui/LoadingSpinner';
import { generateJson } from '../services/aiService';
import { OPERATOR_TEMPLATES, RST_TRAITS } from '../constants';
...
interface OperatorBuilderViewProps {
  ...
  personas: Persona[];
  operators: Operator[];
  onAddOperator: (operatorData: Omit<Operator, 'id' | 'user_id' | 'created_at' | 'updated_at'> & Partial<AIOperatorEffectivenessAnalysis>) => void;
  onUpdateOperator: (operatorId: number, operatorData: Partial<Omit<Operator, 'id' | 'user_id' | 'created_at'>> & Partial<AIOperatorEffectivenessAnalysis>) => void;
            ...
            <div className="flex justify-between items-center">
                ...
                {score !== null && score !== undefined && (
                     <div className="flex flex-col items-center">
                        ...
                        <span className="text-xxs text-muted-foreground -mt-1">Score</span>
                    </div>
                )}
            </div>
        </button>
...
export const OperatorBuilderView: React.FC<OperatorBuilderViewProps> = ({ currentUser, personas, operators, onAddOperator, onUpdateOperator, onDeleteOperator, onNavigate }) => {
  ...
  const handleApplyTemplate = (templateData: Partial<Omit<Operator, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'target_audience_id'>>) => {
      handleCreateNew();
      // This is a bit of a trick to ensure the form component re-mounts with the template data
  ...
  const handleAnalyzeOperator = useCallback(async (operatorData: Operator, persona: Persona) => {
      ...
      const prompt = `
        Analyze the effectiveness of a psychological operator on a target persona.
        
        Persona Details:
        - RST Profile: BAS: ${rstProfile?.bas}, BIS: ${rstProfile?.bis}, FFFS: ${rstProfile?.fffs}
        - Demographics: ${persona.demographics}
        - Psychographics: ${persona.psychographics}

        Operator Details:
        - Type: ${operatorData.type}
        - Desired Response: ${operatorData.desired_conditioned_response}
        - Conditioned Stimulus: ${operatorData.conditioned_stimulus}
        - Unconditioned Stimulus: ${operatorData.unconditioned_stimulus}
        - Reinforcement Loop: ${operatorData.reinforcement_loop}

        Provide a JSON response with:
        1. "effectivenessScore": An integer score from 0 to 100.
        2. "alignmentAnalysis": A concise paragraph explaining how well the operator type and stimuli align with the persona's RST profile and psychology.
        3. "improvementSuggestions": An array of 2-3 actionable, concrete suggestions to strengthen the operator's impact.
      `;
      ...
      if (result.data) {
          const analysisData = {
              effectiveness_score: result.data.effectivenessScore,
              alignment_analysis: result.data.alignmentAnalysis,
              improvement_suggestions: result.data.improvementSuggestions,
          };
        ...
        <div className="lg:col-span-1">
          <Card title="Your Operators">
            <div className="space-y-2">
              <Button onClick={handleCreateNew} variant="primary" className="w-full mb-2" leftIcon={<PlusCircleIcon className="w-4 h-4" />} disabled={showPrerequisiteMessage}>
                  Create New Operator
              </Button>
              <Button onClick={() => setIsTemplatesModalOpen(true)} variant="secondary" className="w-full" leftIcon={<DocumentDuplicateIcon className="w-4 h-4" />} disabled={showPrerequisiteMessage}>
                  Use a Template
              </Button>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-border space-y-1 max-h-96 overflow-y-auto">
              {sortedOperators.map(op => (
                <OperatorCard 
                  key={op.id}
                  operator={op}
                  personaName={personas.find(p => p.id === op.target_audience_id)?.name || "N/A"}
                  onSelect={() => handleSelectOperator(op)}
                  isActive={selectedOperator?.id === op.id}
                />
              ))}
              {operators.length === 0 && !showPrerequisiteMessage && <p className="text-sm text-center text-muted-foreground py-4">No operators created yet.</p>}
            </div>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2">
          {!selectedOperator && !isCreatingNew && (
            <Card className="flex items-center justify-center h-full min-h-[400px] text-center">
              <p className="text-muted-foreground">Select an operator to view its details, or create a new one.</p>
            </Card>
          )}
          ...
          {(selectedOperator || isCreatingNew) && (
            <div className="space-y-6">
              ...
              <Card title="Operator Flow Diagram">
                 <OperatorFlowDiagram operator={selectedOperator} persona={personas.find(p => p.id === selectedOperator?.target_audience_id)} />
              </Card>
                      ...
                      {!isLoading && selectedOperator.effectiveness_score !== null && selectedOperator.effectiveness_score !== undefined ? (
                          <div className="space-y-4">
                              <div className="flex flex-col md:flex-row items-center gap-6">
                                  <EffectivenessGauge score={selectedOperator.effectiveness_score} />
                                  <div className="flex-1">
                                      <h4 className="font-semibold text-foreground mb-1">Alignment Analysis</h4>
                                      <p className="text-sm text-muted-foreground">{selectedOperator.alignment_analysis}</p>
                                  </div>
                              </div>
                              <div>
                                  <h4 className="font-semibold text-foreground mb-2">Improvement Suggestions</h4>
                                  <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                                      {(selectedOperator.improvement_suggestions || []).map((s, i) => <li key={i}>{s}</li>)}
                                  </ul>
                              </div>
                          </div>
                      ) : (
