...
const AiProviderConfigTab: React.FC = () => {
    ...
    const [globalDefaultVideoModel, setGlobalDefaultVideoModel] = useState<string>('');
    ...
    const loadConfigs = useCallback(async () => {
        ...
        setGlobalDefaultVideoModel(globalSettings.global_default_video_model || '');
    ...
    const handleModelListChange = (providerId: AiProviderType, modelType: 'text' | 'image' | 'chat' | 'embedding' | 'video', value: string) => {
        setProviderConfigs(prev => 
            prev.map(p => {
                if (p.id === providerId) {
                    const newModels = { ...p.models };
                    (newModels as any)[modelType] = value.split('\n').map(line => line.trim()).filter(line => line);
    ...
    const handleSaveConfigs = async () => {
        try {
            ...
            const globalSettingsPayload: Database['public']['Tables']['app_global_settings']['Update'] = {
                ...
                global_default_video_model: globalDefaultVideoModel || null,
                updated_at: new Date().toISOString()
            };
    ...
    const allVideoModelOptions = useMemo(() => {
        ...
        providerConfigs.forEach(provider => {
            const labelSuffix = provider.is_enabled ? '' : ' (Disabled)';
            (provider.models?.video || []).forEach(model => {
                options.push({ value: model, label: `${provider.name}: ${model}${labelSuffix}`});
            });
        });
        return options;
                ...
                <div className="space-y-4 mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Select
                            ...
                            value={globalDefaultVideoModel}
                            onChange={(e) => setGlobalDefaultVideoModel(e.target.value)}
                            containerClassName="mb-0"
                        />
                    </div>
                </div>
                                ...
                                <>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-border">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                                            {providerTemplate?.models.text && ( <Textarea label="Text Models" value={(provider.models.text || []).join('\n')} onChange={(e) => handleModelListChange(provider.id, 'text', e.target.value)} rows={5} className="font-mono text-xs" containerClassName="mb-0" /> )}
                                            {providerTemplate?.models.image && ( <Textarea label="Image Models" value={(provider.models.image || []).join('\n')} onChange={(e) => handleModelListChange(provider.id, 'image', e.target.value)} rows={5} className="font-mono text-xs" containerClassName="mb-0" /> )}
                                            {providerTemplate?.models.chat && ( <Textarea label="Chat Models" value={(provider.models.chat || []).join('\n')} onChange={(e) => handleModelListChange(provider.id, 'chat', e.target.value)} rows={5} className="font-mono text-xs" containerClassName="mb-0" /> )}
                                            {providerTemplate?.models.embedding && ( <Textarea label="Embedding Models" value={(provider.models.embedding || []).join('\n')} onChange={(e) => handleModelListChange(provider.id, 'embedding', e.target.value)} rows={5} className="font-mono text-xs" containerClassName="mb-0" /> )}
                                            {providerTemplate?.models.video && ( <Textarea label="Video Models" value={(provider.models.video || []).join('\n')} onChange={(e) => handleModelListChange(provider.id, 'video', e.target.value)} rows={5} className="font-mono text-xs" containerClassName="mb-0" /> )}
                                        </div>
                                    </div>
                                </>
                            )}
