...
serve(async (req: Request)=>{
  ...
  try {
    ...
    if (!text || typeof text !== 'string' || !voiceId || typeof voiceId !== 'string' || !engine || typeof engine !== 'string') {
      return new Response(JSON.stringify({
        error: 'Invalid request body. Requires { text: string, voiceId: string, engine: string }.'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    const pollyClient = initializePollyClient();
    if (!pollyClient) {
      return new Response(JSON.stringify({
        error: 'Service misconfigured: AWS credentials not set on server.'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    // Use the robust manual stream handler instead of the SDK's transformToByteArray
    ...
    return new Response(JSON.stringify({
      error: 'An internal server error occurred during speech synthesis.'
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
